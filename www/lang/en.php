...
<p>You are <i><?=$user['user_name'] ?></i> (<a href="/logout">log out</a>) and <?php if( is_null($user['last_use']) ) { ?>you have never used a token<?php } else { ?>the last time you have used a token was on <i><?=$user['last_use'] ?> UTC</i><?php } ?>.
...
If a token has been compromised (that is, someone learned it), just <a href="/newmaster">reissue</a> it<?php if( isset($master['token']) ) { ?> (or <a href="/nomaster">disable</a> it altogether)<?php } ?>.
<?php if( isset($master['last']) ) { ?>This token has been last used on <i><?=$master['last'] ?> UTC</i>.<?php } ?></p>
...
<?php } else { ?><div class="token"><i>Token is disabled. <a href="/newmaster">Enable</a></i><?php } ?>
...
<?php } else { ?><div class="token"><i>No tokens left. <a href="/newtokens">Make more</a></i><?php } ?>
    </div>
