<?php // OSM Auth Proxy web interface, English translation. Written by Ilya Zverev, licensed WTFPL
...
<p>Вы <i><?=$user['user_name'] ?></i> (<a href="/logout">выйти</a>), и <?php if( is_null($user['last_use']) ) { ?>вы ни разу не воспользовались токеном<?php } else { ?>последний раз вы пользовались токеном <i><?=$user['last_use'] ?> UTC</i><?php } ?>.
...
Если токен скомпрометирован (то есть, кто-то его узнал), просто <a href="/newmaster">перевыпустите</a> его<?php if( isset($master['token']) ) { ?> (или вообще <a href="/nomaster">отключите</a>)<?php } ?>.
<?php if( isset($master['last']) ) { ?>Этим токеном в последний раз пользовались <i><?=$master['last'] ?> UTC</i>.<?php } ?></p>
<?php if( isset($master['token']) ) { ?>
...
<?php } else { ?><div class="token"><i>Токен отключен. <a href="/newmaster">Включить</a></i><?php } ?>
...
<div class="hidelink" onclick="javascript:replace(this, 'onetime');">Показать одноразовые токены</div>
    ...
    <?php foreach( $tokens as $token ) { ?><div><?=$token ?></div><?php } ?></div>
<?php } else { ?><div class="token"><i>Токены закончились. <a href="/newtokens">Сделать ещё</a></i><?php } ?>
    </div>
